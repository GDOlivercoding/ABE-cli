...
def Overpower(atk: Attack, self: Ally, target: Enemy):
    ...
    target.deal_damage(
        damage, self, [atk.sbm(DamageDebuff, turns=2, effectiveness=25)]
    )
...
def Aura_Of_Fortitude(sprt: Support, self: Ally, target: Enemy):
    for ally in self.battle.allied_units.values():
        ally.add_pos_effects(
            sprt.sbm(Shield, turns=4, effectiveness=25)
        )
...
def Defensive_Formation(sprt: Support, self: Ally, target: Enemy):
    for ally in self.battle.allied_units.values():
        if ally.is_same(target):
            ally.add_pos_effects(
                sprt.sbm(Shield, turns=1, effectiveness=50)
            )
            continue
        ally.add_pos_effects(
            sprt.sbm(Shield, turns=1, effectiveness=40)
        )
...
def _Devotion(sprt: Support, self: Ally, target: Enemy):
    target.add_pos_effects(
        sprt.sbm(Devotion, turns=3, protector=self, shield=40)
    )
...
def Matildas_medicine(chili: Chili, self: Ally):
    ...
    for unit in battle.allied_units.values():
        unit.cleanse()
        unit.heal(PercDmgObject(unit.TOTAL_HP) % 35) # TODO: 35 should be in the value index
...
def Egg_Surprise(chili: Chili, self: Ally):
    ...
    random.choice([*battle.enemy_units.values()]).add_neg_effects(chili.sbm(Knock, turns=1))
